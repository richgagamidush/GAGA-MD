...
zokou({
  ...
  if (arg[0]) {
    ...
    if (action === "on") {
      ...
      await zk.sendMessage(origineMessage, "Antidelete successfully activated.");
      return;
    } else if (action === "off") {
      antiDeleteActive = false;
      await zk.sendMessage(origineMessage, "Antidelete successfully deactivated.");
      return;
    }
  }
  ...
  if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ADM).toLowerCase() === 'yes') {
    ...
    try {
      ...
      const caption = ` Anti-delete-message detect by  Gaga md\nMessage de @${senderId}`;
